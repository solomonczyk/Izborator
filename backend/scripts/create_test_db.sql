-- Создание тестовой базы данных для Izborator
-- Выполните: docker exec -i izborator_postgres psql -U postgres < backend/scripts/create_test_db.sql

-- Создаём базу данных (если не существует)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'izborator') THEN
        CREATE DATABASE izborator;
    END IF;
END $$;
